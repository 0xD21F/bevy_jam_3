...
pub fn wander(
    ...
    for (mut velocity, creature, mut wander, transform) in wander_query.iter_mut() {
        ...
        if wander.wander_node.is_none()
            || unit_position.distance(wander.wander_node.unwrap()) < REACHED_WANDER_NODE_DISTANCE
        {
            wander.wander_node = Some(
                unit_position
                    + Vec2::new(rng.gen_range(-1.0..1.0), rng.gen_range(-1.0..1.0))
                        .normalize_or_zero()
                        * NEXT_WANDER_NODE_DISTANCE,
            );
        }
        ...
        velocity.value +=
            direction.normalize_or_zero() * creature.acceleration * time.delta_seconds();
    }
}
